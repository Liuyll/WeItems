...
class ThirdPartyLoginManager {
    ...
    // MARK: - 6. 第三方 provider_token 注册（直接方式）
    
    /// 使用 provider_token 进行第三方授权注册
    private func signupWithProviderToken(
        providerToken: String,
        username: String,
        password: String
    ) async throws -> SignupResponse {
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            if let errorResponse = try? JSONDecoder().decode(SignupErrorResponse.self, from: data) {
                if errorResponse.error == "user_already_exists" {
                    throw ThirdPartyLoginError.userAlreadyExists
                }
                throw ThirdPartyLoginError.serverError(errorResponse.errorDescription ?? errorResponse.error)
            }
            throw ThirdPartyLoginError.serverError("HTTP \(httpResponse.statusCode)")
    ...
    private func loginWithPassword(
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            throw ThirdPartyLoginError.serverError("密码登录失败 HTTP \(httpResponse.statusCode)")
        }
